...
另一方面，对于房价较高的观察值，其预测值可能会有较大的误差；而对于房价较低的观察值的预测值的误差也可能较低。为了消除其影响，实际上进行评价的指标是房价准确值与预测值的对数值的RMSE。
...
在使用该模型预测测试集时，有部分数据显示为NA，这与部分观察值某项指标的缺失有关。考虑使用训练集的均值取代NA作为最终提交结果，在比赛网站上得到最终评价指标为0.38025[[5]](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/leaderboard#score)，虽高于训练集的指标，但仍是一个可以接受的结果。
